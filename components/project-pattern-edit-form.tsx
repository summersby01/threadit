...
export function ProjectPatternEditForm({ projectId }: { projectId: string }) {
  ...
  const setProjectPatternAssetIds = usePatternRowsStore(
    (state) => state.setProjectPatternAssetIds,
  );
        ...
        <PageCard>
          <div className="rounded-[1.5rem] border border-cream-200 bg-cream-50 p-5">
            ...
            <button
              type="button"
              onClick={() => router.push("/")}
              className="pill-button mt-4 h-10 px-4"
            >
    ...
    <section className="space-y-7 sm:space-y-8">
      <div className="flex items-start justify-between gap-4">
        ...
        <button
          ...
          className="pill-button h-11 px-4"
        >
      ...
      <PageCard>
        <PatternAssetPicker
          ...
          onChange={(ids) => setProjectPatternAssetIds(projectId, ids)}
        />
      </PageCard>
